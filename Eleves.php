...
function displayError($error) {
    if (!empty($error)) {
        ...
        echo '<div class="alert alert-warning alert-dismissible fade show" role="alert" >' . $error . '</div>';
        echo '</p>';
    }
}
...
if (isset($_POST['subnit'])) {
    ...
    if ($result === false) {
        displayError("La classe '$lib_class' n'existe pas dans la base de données.");
    } else {
    ...
    if (!empty($student_name) && !empty($student_surname) && !empty($stud_brthd) && !empty($stud_sexe) && !empty($student_phone) && !empty($student_email) && !empty($id_class)) {
        ...
        if ($phoneExists > 0) {
            displayError("Le numéro de téléphone existe déjà.");
        }

        if ($emailExists > 0) {
            displayError("L'email existe déjà.");
        }
        ...
        try {
            ...
            if ($stmt->execute()) {
                displayError("Erreur lors de l'ajout de l'élève.");
            } else {
                displayError("Erreur lors de l'ajout du professeur.");
            }
    ...
    <div id="right-panel" class="right-panel">
        ...
        <form class="needs-validation" novalidate action="" method="POST" style="box-sizing: border-box;">
            <div class="form-row" style="margin-left: 5px; margin-right: 5px; margin-bottom: -30; ">
                ...
                <div class="col-md-4 mb-3">
                    <label for="validationTooltipUsername">classe</label>
